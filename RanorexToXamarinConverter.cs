...
public class RanorexToXamarinConverter
{
    ...
    private string _currentInputFile = string.Empty;
    ...
    public void ConvertSingleFile(string inputPath)
    {
        ...
        _currentInputFile = inputPath;  // Set this before any conversion
    ...
    private List<TestCase> ParseTestCases(XDocument testSuite)
    {
        ...
        foreach (var element in testCaseElements)
        {
            ...
            {
                LogError($"Test case missing required name or path attribute: {element}");
                continue;
            }
    ...
    {
        return $@"// Generated by RanorexToXamarinConverter on {DateTime.Now:yyyy-MM-dd HH:mm:ss}
// Original file converted: {_currentInputFile}
...
";
    ...
    private void ConvertTestCase(TestCase testCase)
    {
        ...
        xamarinTest.Append(GetFileHeaderComment());
        ...
        string targetPath = Path.Combine(_outputPath, $"{testCase.Name}Tests.cs");
        File.WriteAllText(targetPath, xamarinTest.ToString());
        LogMessage($"Created file: {targetPath}");
    }
...
{{
    ...
    {
        _currentInputFile = rxrecPath;  // Store current file for header comment
        ...
        LogMessage($"Created file: {outputPath}");
    }
    ...
    {
        _currentInputFile = csharpPath;  // Store current file for header comment
        Directory.CreateDirectory(_outputPath);
        
        var code = new StringBuilder();
        code.Append(GetFileHeaderComment());
        code.Append(ConvertRanorexCode(File.ReadAllText(csharpPath)));
        
        string outputFileName = Path.GetFileName(csharpPath);
        string targetPath = Path.Combine(_outputPath, outputFileName);
        File.WriteAllText(targetPath, code.ToString());
        LogMessage($"Created file: {targetPath}");
    }
    ...
    {
        _currentInputFile = "BaseTestFixture template";  // Store current file for header comment
        var baseTest = new StringBuilder();
        baseTest.Append(GetFileHeaderComment());
        baseTest.Append(@"
...
namespace XamarinTests
{
    public class BaseTestFixture
    {
        ...
        string targetPath = Path.Combine(_outputPath, "BaseTestFixture.cs");
        File.WriteAllText(targetPath, baseTest.ToString());
        LogMessage($"Created file: {targetPath}");
    }
